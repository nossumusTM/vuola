...
import { availableTimes } from '../inputs/Calendar';
...
const ListingReservation: React.FC<ListingReservationProps> = ({
  ...
  const handleReserve = () => {
    ...
    // ✅ Check if selected time is in that day's available slots
    const isTimeAvailable = availableTimesForDay.includes(normalizedTime);
    ...
    const isPast = isToday && new Date() > selectedTimeDate;
  
    if (!isTimeAvailable || isPast) {
      toast.error("Selected time is not available. Please choose another.");
      return;
    }
